...
- **context(業務背景)およびexplanation(解説)は、Markdown記法を最大限活用して構造化してください。**
  ...
  - 重要：ただし、JSONの \`problemCode\` および \`solutionCode\` フィールド内には、基本的には純粋なコードのみを出力してください。
  - **ただし、複数のファイル（例: Controller, Service, Modelなど）にまたがる問題を出題したい場合に限り**、各ファイルを \`\`\`language:filename.ext\`\`\` という形式で複数記述することを許可します。この際、バックティックの間に余計なテキストを入れないでください。
