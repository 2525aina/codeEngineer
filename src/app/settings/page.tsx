...
import { Save, Trash2, CheckCircle, AlertCircle, ShieldCheck, Zap, Sparkles, Loader2, Cpu, ChevronRight, PlusCircle } from "lucide-react";
...
export default function Settings() {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-12 pb-32 animate-reveal">
            {/* Header */}
            <div className="space-y-4 text-center">
                ...
                <h1 className="text-4xl md:text-7xl font-black tracking-tighter">
                    AIエンジンを<span className="text-google-gradient italic">最適化</span>する
                </h1>
                <p className="text-secondary text-lg max-w-2xl mx-auto opacity-80">
                    モデルごとに個別の API キーを割り当て可能。
                    用途に応じて最適なエンジンを構成してください。
                </p>
            </div>
            ...
            {/* Bulk Setup Card */}
            <div className="glass rounded-[40px] border p-8 md:p-10 space-y-6 bg-google-ai border-primary/20 shadow-xl shadow-primary/5">
            ...
            <div className="grid grid-cols-1 gap-6">
                {MODELS.map((model) => (
                    <div key={model.id} id={`model-${model.id}`} className="glass rounded-[32px] border p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center bg-google-ai overflow-hidden relative scroll-mt-24">
                        {/* Model Info */}
